...
class PurchaseJournalDialog(QDialog):
    def __init__(self, db_manager, parent=None, entry_data=None, is_copy=False):
        ...
        self.user_lines    = []
        self.alphalist_map = {}
        self._ap_code = ''; self._ap_desc = 'Accounts Receivable'
        ...
        if entry_data: self.branch_input.setText(entry_data.get("branch_code","") or "")
        ...
        ar_info = QLabel("  (AP will be auto-computed as the balancing credit)")
    ...
    def _load_payees(self):
        ...
        for entry in self.db_manager.get_all_alphalist():
            if entry.get("entry_type","Customer") not in ("Vendor","Customer&Vendor"): continue
    ...
    def _refresh_lines(self):
        ...
        ar_amount = total_dr - total_cr  # AP = balancing credit

        all_lines = list(self.user_lines)
        ar_line = None
        if ar_amount > 0:
            ar_line = {"account_description": self._ap_desc, "account_code": self._ap_code,
        ...
        if ar_amount > 0:
            self.ar_lbl.setText(f"  AP (Balancing Credit): {ar_amount:,.2f}")
    ...
    def _on_save(self):
        if not self.customer_input.currentText().strip():
            QMessageBox.warning(self,"Validation","Payee Name is required."); return
    ...
    def get_data(self):
        ...
        if ar_amount > 0:
            lines.append({"account_description": self._ap_desc, "account_code": self._ap_code,
                          "debit": 0, "credit": ar_amount})
        return {
...
class _ViewPJDialog(QDialog):
    def __init__(self, parent, entry):
        ...
        info = QLabel(
            f"<b>Date:</b> {entry['date']}  &nbsp; <b>Payee:</b> {entry['payee_name']}  &nbsp; "
...
class PurchaseJournalWidget(QWidget):
    ...
    def _setup_ui(self):
        layout = QVBoxLayout()
        title = QLabel("PURCHASE JOURNAL"); title.setProperty("class","title"); title.setAlignment(Qt.AlignCenter); layout.addWidget(title)
        ...
        br.addStretch()
        self.totals_label = QLabel("Totals: Gross: 0.00 | Input VAT: 0.00"); self.totals_label.setProperty("class","total"); br.addWidget(self.totals_label)
        layout.addLayout(br)

        self.table = QTableWidget(); self.table.setColumnCount(6)
        self.table.setHorizontalHeaderLabels(["Date","Payee Name","Reference No.","TIN","Lines","Total Gross"])
    ...
    def _populate_table(self):
        ...
        for r, entry in enumerate(self.all_entries):
            ...
            for c, text in [(1,entry.get("payee_name","")), (2,entry.get("reference_no","")), (3,str(entry.get("tin","") or ""))]:
    ...
    def _update_totals(self):
        ...
        for r in range(self.table.rowCount()):
            ...
            if entry:
                gross += entry.get("gross_amount",0); vat   += entry.get("input_vat",0)
        self.totals_label.setText(f"Totals: Gross: {gross:,.2f} | Input VAT: {vat:,.2f}")
    ...
    def _view_details(self):
        ...
        _ViewPJDialog(self, entry).exec_()
    ...
    def _export_xls(self):
        ...
        path, _ = QFileDialog.getSaveFileName(self, "Export Purchase Journal", os.path.join(get_io_dir("Purchase Journal"), "purchase_journal.xlsx"), "Excel Files (*.xlsx)")
        if not path: return
        rows = []
        for r in range(self.table.rowCount()):
            if self.table.isRowHidden(r): continue
            ...
            for ln in entry.get("lines",[]):
                rows.append({"date": entry["date"], "payee_name": entry["payee_name"],
    ...
    def _import_xls(self):
        if not _OPENPYXL_OK: QMessageBox.critical(self,"Missing Library","openpyxl required."); return
        path, _ = QFileDialog.getOpenFileName(self, "Import Purchase Journal", get_import_dir(""), "Excel Files (*.xlsx *.xls)")
        if not path: return
...
def _import_from_xls(path, db_manager):
    ...
    for key in order:
        ...
        first=groups[key][0]
        data = {
            "date": date, "payee_name": _val(first,"payee_name"),
