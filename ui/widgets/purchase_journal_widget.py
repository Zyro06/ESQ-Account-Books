...
from ui.dialogs.view_details_dialog import ViewEntryDialog  as _ViewPJDialog
...
_XLS_COLS = [
    ('Date','date'), ('Payee Name','payee_name'), ('Reference No','reference_no'),
...
class PurchaseJournalDialog(QDialog):
    def __init__(self, db_manager, parent=None, entry_data=None, is_copy=False):
        ...
        self.user_lines    = []
        self.alphalist_map = {}
        self._ap_code = ''; self._ap_desc = 'Accounts Payable'

        title = ("Copy Entry" if is_copy else
                 "Add Purchase Entry" if entry_data is None else "Edit Purchase Entry")
        ...
        self._load_payees()
        hdr.addRow("Payee Name:", self.customer_input)
        ...
        if entry_data:
            self.branch_input.setText(entry_data.get("branch_code", "") or "")
        hdr.addRow("Branch Code:", self.branch_input)
        ...
        ap_info = QLabel("  (AP will be auto-computed as the balancing credit)")
        ap_info.setStyleSheet("color:#888; font-style:italic;")
        lines_hdr.addWidget(lbl); lines_hdr.addWidget(ap_info); lines_hdr.addStretch()
        ...
        self._find_ap_account()

        if entry_data:
            self.customer_input.setCurrentText(entry_data.get("payee_name", "") or "")
            self.tin_input.setText(str(entry_data.get("tin", "") or ""))
            for ln in entry_data.get("lines", []):
                if _numeric_suffix(ln.get("account_code", "")) != _AP_NUM:
                    self.user_lines.append(dict(ln))
        self._refresh_lines()

    def _find_ap_account(self):
        for acct in self.db_manager.get_all_accounts():
            if _numeric_suffix(acct["account_code"]) == _AP_NUM:
                self._ap_code = acct["account_code"]
                self._ap_desc = acct["account_description"]
                break

    def _load_payees(self):
        self.customer_input.addItem("")
        for entry in self.db_manager.get_all_alphalist():
            if entry.get("entry_type", "Vendor") not in ("Vendor", "Customer&Vendor"):
                continue
    ...
    def _refresh_lines(self):
        ...
        ap_amount = total_dr - total_cr

        all_lines = list(self.user_lines)
        if ap_amount > 0:
            all_lines.append({
                "account_description": self._ap_desc,
                "account_code": self._ap_code,
                "debit": 0, "credit": ap_amount, "_auto": True})
        ...
        final_cr = total_cr + (ap_amount if ap_amount > 0 else 0)
        self.total_dr_lbl.setText(f"Total Debit:  {total_dr:,.2f}")
        self.total_cr_lbl.setText(f"Total Credit: {final_cr:,.2f}")
        balanced = abs(total_dr - final_cr) < 0.005 and total_dr > 0
        if balanced:
        ...
        if ap_amount > 0:
            self.ap_lbl.setText(f"  AP (Balancing Credit): {ap_amount:,.2f}")
            self.ap_lbl.setStyleSheet("color:#2244aa; font-style:italic;")
        else:
            self.ap_lbl.setText("")
    ...
    def get_data(self):
        ...
        if ap_amount > 0:
            lines.append({"account_description": self._ap_desc,
                          "account_code": self._ap_code,
                          "debit": 0, "credit": ap_amount})
        return {
            "date":        self.date_input.date().toString("MM/dd/yyyy"),
            "payee_name":  self.customer_input.currentText().strip(),
...
class PurchaseJournalWidget(QWidget):
    ...
    def _setup_ui(self):
        ...
        self.search_input.setPlaceholderText(
            "Search by payee, reference, TIN, or particulars...")
        ...
        self.table.setHorizontalHeaderLabels(
            ["Date", "Payee Name", "Reference No.", "TIN", "Lines", "Total Gross"])
        ...
        self.totals_label = QLabel("Totals  |  Gross: 0.00  |  Input VAT: 0.00")
    ...
    def _update_totals(self):
        ...
        for r in range(self.table.rowCount()):
            ...
            if entry:
                gross += entry.get("gross_amount", 0)
                vat   += entry.get("input_vat",    0)
        self.totals_label.setText(
            f"Totals  |  Gross: {gross:,.2f}  |  Input VAT: {vat:,.2f}")
    ...
    def _add_entry(self):
        dlg = PurchaseJournalDialog(self.db_manager, self)
        if dlg.exec():
            if self.db_manager.add_purchase_entry(dlg.get_data()):
                self.load_data()
    ...
    def _edit_entry(self):
        ...
        if not entry:
            ...
            return
        dlg = PurchaseJournalDialog(self.db_manager, self, entry)
        if dlg.exec():
            if self.db_manager.update_purchase_entry(entry["id"], dlg.get_data()):
                self.load_data()
    ...
    def _copy_entry(self):
        ...
        if not entry:
            ...
            return
        dlg = PurchaseJournalDialog(self.db_manager, self, entry, is_copy=True)
        if dlg.exec():
            if self.db_manager.add_purchase_entry(dlg.get_data()):
                self.load_data()
    ...
    def _view_details(self):
        ...
        if not entry:
            ...
            return
        _ViewPJDialog(self, entry, name_label='Payee').exec()
    ...
    def _delete_entry(self):
        ...
        if reply == QMessageBox.Yes:
            for entry in entries:
                self.db_manager.delete_purchase_entry(entry["id"])
            self.load_data()
    ...
    def _export_xls(self):
        ...
        path, _ = QFileDialog.getSaveFileName(
            ...
            os.path.join(get_io_dir("Purchase Journal"), "purchase_journal.xlsx"),
            "Excel Files (*.xlsx)")
        if not path: return
        rows = []
        for r in range(self.table.rowCount()):
            ...
            for ln in entry.get("lines", []):
                rows.append({
                    "date": entry["date"], "payee_name": entry["payee_name"],
        ...
        n, err = _export_to_xls(
            rows, path, "Purchase Journal", columns=_XLS_COLS,
    ...
    def _import_xls(self):
        ...
        try:
            imported, skipped, errors = import_grouped_from_xls(
                ...
                add_method_name='add_purchase_entry',
                columns=_XLS_COLS, payee_key='payee_name')
