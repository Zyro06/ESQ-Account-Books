"""
ui/widgets/cash_receipts_widget.py
-----------------------------------
Cash Receipts Journal widget.
...
The entry dialog (CashReceiptsDialog) is structurally identical to
CashDisbursementDialog — only the window title differs.
"""
...
class CashReceiptsDialog(QDialog):
    """Add / edit / copy a Cash Receipts journal entry."""
    ...
    def __init__(self, db_manager, parent=None, entry_data=None, is_copy=False):
        ...
        if is_copy:
            ...
            self.setWindowTitle('New Entry — Cash Receipts Journal')
        else:
...
class CashReceiptsWidget(BaseJournalWidget):
    TITLE         = 'CASH RECEIPTS JOURNAL'
    DIALOG_CLASS  = CashReceiptsDialog
    GET_METHOD    = 'get_cash_receipts_journal'
    ADD_METHOD    = 'add_cash_receipts_entry'
    DELETE_METHOD = 'delete_cash_receipts_entry'
    EXPORT_FOLDER = 'Cash Receipts'
    EXPORT_FILE   = 'cash_receipts_journal_report.xlsx'
    EXPORT_TITLE  = 'Cash Receipts Journal'
    IMPORT_TITLE  = 'Import Cash Receipts Journal'
