...
class CashDisbursementDialog(QDialog):
    def __init__(self, db_manager, parent=None, entry_data=None, is_copy=False):
        ...
        if is_copy:
            ...
            self.setWindowTitle("New Journal Entry Cash Disbursement Journal")
        else:
...
class CashDisbursementWidget(QWidget):
    ...
    def _setup_ui(self):
        layout = QVBoxLayout()
        title = QLabel("CASH DISBURSEMENT JOURNAL")
    ...
    def load_data(self):
        self.all_entries = self.db_manager.get_cash_disbursement_journal()
    ...
    def _add_entry(self):
        dialog = CashDisbursementDialog(self.db_manager, self)
        if dialog.exec_():
            rows = dialog.get_data()
            ok = all(self.db_manager.add_cash_disbursement_entry(r) for r in rows)
            self.load_data()
    ...
    def _edit_entry(self):
        ...
        dialog = CashDisbursementDialog(self.db_manager, self, group)
        if not dialog.exec_(): return
        new_rows = dialog.get_data()
        for old_id in group['ids']:
            self.db_manager.delete_cash_disbursement_entry(old_id)
        ok = all(self.db_manager.add_cash_disbursement_entry(r) for r in new_rows)
        self.load_data()
        (QMessageBox.information if ok else QMessageBox.warning)(
            self, "Success" if ok else "Error",
    ...
    def _copy_entry(self):
        ...
        dialog = CashDisbursementDialog(self.db_manager, self, copy, is_copy=True)
        if not dialog.exec_(): return
        rows = dialog.get_data()
        ok = all(self.db_manager.add_cash_disbursement_entry(r) for r in rows)
        self.load_data()
    ...
    def _export_xls(self):
        ...
        path, _ = QFileDialog.getSaveFileName(
            self, "Export Cash Disbursement Journal",
            os.path.join(get_io_dir("Cash Disbursement"), "cash_disbursement_journal_report.xlsx"),
            "Excel Files (*.xlsx)")
        if not path: return
        rows = []
        ...
        n, err = _export_to_xls(rows, path, "Cash Disbursement Journal")
    ...
    def _import_xls(self):
        ...
        path, _ = QFileDialog.getOpenFileName(
            self, "Import Cash Disbursement Journal", get_import_dir(""), "Excel Files (*.xlsx *.xls)")
        if not path: return
        try:
            imported, skipped, errors = _import_from_xls(path, self.db_manager, 'add_cash_disbursement_entry')
