...
class GeneralJournalDialog(QDialog):
    def __init__(self, db_manager, parent=None, entry_data=None, is_copy=False):
        super().__init__(parent)
        self.db_manager = db_manager
        self.lines      = []
        if is_copy:
            self.setWindowTitle("Copy Entry General Journal")
        elif entry_data is None:
            self.setWindowTitle("New Journal Entry General Journal")
        else:
            self.setWindowTitle("Edit Entry General Journal")
        self.setModal(True)
...
class GeneralJournalWidget(QWidget):
    ...
    def load_data(self):
        self.all_entries = self.db_manager.get_general_journal()
    ...
    def _add_entry(self):
        dialog = GeneralJournalDialog(self.db_manager, self)
        if dialog.exec_():
            rows = dialog.get_data()
            ok = all(self.db_manager.add_general_journal_entry(r) for r in rows)
            self.load_data()
    ...
    def _edit_entry(self):
        ...
        dialog = GeneralJournalDialog(self.db_manager, self, group)
        if not dialog.exec_(): return
        new_rows = dialog.get_data()
        for old_id in group['ids']:
            self.db_manager.delete_general_journal_entry(old_id)
        ok = all(self.db_manager.add_general_journal_entry(r) for r in new_rows)
        self.load_data()
        (QMessageBox.information if ok else QMessageBox.warning)(
            self, "Success" if ok else "Error",
    ...
    def _copy_entry(self):
        ...
        dialog = GeneralJournalDialog(self.db_manager, self, copy, is_copy=True)
        if not dialog.exec_(): return
        rows = dialog.get_data()
        ok = all(self.db_manager.add_general_journal_entry(r) for r in rows)
        self.load_data()
    ...
    def _export_xls(self):
        ...
        path, _ = QFileDialog.getSaveFileName(
            self, "Export General Journal",
            os.path.join(get_io_dir("General Journal"), "general_journal_report.xlsx"),
            "Excel Files (*.xlsx)")
        if not path: return
        rows = []
        ...
        n, err = _export_to_xls(rows, path, "General Journal")
    ...
    def _import_xls(self):
        ...
        path, _ = QFileDialog.getOpenFileName(
            self, "Import General Journal", get_import_dir(""), "Excel Files (*.xlsx *.xls)")
        if not path: return
        try:
            imported, skipped, errors = _import_from_xls(path, self.db_manager, 'add_general_journal_entry')
